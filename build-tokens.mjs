...
function emitPaletteLayer(scheme, selector) {
  const lines = [];
  for (const [key, value] of Object.entries(palette)) {
    lines.push(`  --md-ref-palette-${key}: ${value};`);
  }
  return `${selector} {\n${lines.join('\n')}\n}\n`;
}
...
function emitColorVars(scheme, selector) {
  ...
  for (const [role, ref] of Object.entries(roles)) {
    // Map roles to palette variables instead of direct hex values
    const value = palette[ref] !== undefined
      ? `var(--md-ref-palette-${ref})`
      : ref;
...
function build() {
  ...
  // Emit palette layer (reference tokens)
  const lightPaletteBlock = emitPaletteLayer('light', ':root');
  const darkPaletteBlock = emitPaletteLayer('dark', '[data-theme="dark"]');

  // Emit system color layer (roles mapped to palette)
  const lightColorBlock = emitColorVars('light', ':root');
  const darkColorBlock = emitColorVars('dark', '[data-theme="dark"]');

  // Extract sys-color lines for merging with non-color tokens
  const lightColorLines = lightColorBlock.match(/^\s*--md-sys-color[^\n]+/gm) || [];
