...
?>
<a href="<?php the_permalink(); ?>" class="block group rounded-lg bg-surface-low p-spacing-6 shadow-elevation-1 hover:bg-surface-high hover:shadow-elevation-2 transition-all duration-short ease-standard">
	<?php
	$display_img = jagawarta_get_post_display_image( get_post() );
	if ( ! empty( $display_img['url'] ) ) :
		?>
		<div class="jagawarta-img-wrap jagawarta-img-wrap--hero mb-spacing-6 aspect-[16/9]">
	...
	<p class="mt-spacing-3 text-label-medium text-on-surface-variant"><?php echo esc_html( jagawarta_format_date() ); ?></p>
	<?php
	...
	?>
	<?php if ( has_excerpt() ) : ?>
		<p class="mt-spacing-4 text-body-medium text-on-surface-variant"><?php echo wp_kses_post( get_the_excerpt() ); ?></p>
	<?php endif; ?>
</a>
