...
<article class="group relative flex flex-col h-full min-w-0 overflow-hidden rounded-md bg-surface-low shadow-elevation-1 transition-all duration-medium ease-emphasized hover:bg-surface-high hover:shadow-elevation-3">
	<a href="<?php the_permalink(); ?>" class="flex flex-col h-full focus:outline-none focus:ring-2 focus:ring-primary rounded-md" aria-label="<?php the_title_attribute(); ?>">
		...
		<div class="relative flex flex-col flex-grow px-spacing-8 pt-spacing-6 pb-spacing-10">
			<h3 class="mb-spacing-6 text-title-medium font-medium leading-tight text-on-surface group-hover:text-primary transition-colors duration-short">
				<?php the_title(); ?>
			</h3>

			<div class="mt-auto flex items-center justify-between gap-spacing-2 pt-spacing-6 text-label-medium text-on-surface-variant">
				<span>
					<?php if ($author): ?>
						<span class="font-medium text-on-surface"><?php echo esc_html($author); ?></span>
			...
			<div aria-hidden="true" class="absolute bottom-spacing-10 right-spacing-8 flex items-center h-6 text-on-surface transition-all duration-short ease-in group-hover:right-spacing-5 group-hover:text-primary">
